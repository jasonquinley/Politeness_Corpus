...
from politeness2.constants import POSITIVE_WORDS_PATH, NEGATIVE_WORDS_PATH
...
positive_words = set(
    map(lambda x: x.strip(),
        codecs.open(POSITIVE_WORDS_PATH, encoding='utf-8').read().splitlines())
)
negative_words = set(
    map(lambda x: x.strip(),
        codecs.open(NEGATIVE_WORDS_PATH, encoding='utf-8').read().splitlines())
)
